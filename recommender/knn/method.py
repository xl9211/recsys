...
from recommendation.recommender.knn.item_strategy import NearestItemStrategy
from recommendation.recommender.knn.user_strategy import NearestUserStrategy
...
class ItemBasedRecommender(ItemRecommender):
    ...
    #>>> item_strategy = ItemsNeighborhoodStrategy()
    #>>> similarity = ItemSimilarity(model, euclidean_distances)
    #>>> recsys = ItemBasedRecommender(model, similarity, item_strategy)
    ...
    """
    def __init__(self, model, similarity, item_strategy=None, capper=True, with_preference=False):
        ItemRecommender.__init__(self, model, with_preference)
        self.similarity = similarity
        self.capper = capper
        if item_strategy is None:
            self.item_strategy = NearestItemStrategy()
        else:
    ...
    def all_other_items(self, user_id, **params):
        ...
        """
        return self.item_strategy.candidate_items(user_id, self.model)
...
class UserBasedRecommender(UserRecommender):
    ...
    """
    def __init__(self, model, similarity, user_strategy=None, capper=True, with_preference=False):
        UserRecommender.__init__(self, model, with_preference)
        self.similarity = similarity
        self.capper = capper
        if user_strategy is None:
            self.user_strategy = NearestUserStrategy()
        else:
    ...
    def all_other_items(self, user_id, **params):
        ...
        nearest_neighbors = self.user_strategy.candidate_users(
            user_id,
            self.model,
            n_similarity,
            distance,
