...
import numpy as np
import logging
from recommendation.umatrix.base import BaseDataModel, UserNotFoundError, ItemNotFoundError
...
class MatrixPreferenceDataModel(BaseDataModel):
    ...
    def build_model(self):
        ...
        self.index = np.zeros(shape=(self._user_ids.size, self._item_ids.size))
...
class MatrixBooleanPrefDataModel(BaseDataModel):
    ...
    def build_model(self):
        ...
        self.index = np.zeros(shape=(self._user_ids.size, self._item_ids.size), dtype=bool)
        for user_no, user_id in enumerate(self._user_ids):
            if user_no % 2 == 0:
                logger.debug("PROGRESS: at user_id #%i/%i" % (user_no, self._user_ids.size))
            for item_no, item_id in enumerate(self._item_ids):
